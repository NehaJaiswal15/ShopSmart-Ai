...
def train_baseline(X, y):
    ...
    # Log parameters and metrics to MLflow if run is active
...
def save_model(model, tfidf, metrics):
    ...
    if mlflow.active_run():
        mlflow.log_artifact(os.path.join(MODEL_DIR, "baseline_model.joblib"))
        mlflow.log_artifact(os.path.join(MODEL_DIR, "tfidf_vectorizer.joblib"))
        mlflow.log_artifact(os.path.join(MODEL_DIR, "baseline_metrics.json"))
...
if __name__ == "__main__":
    ...
    with mlflow.start_run(experiment_id=experiment_id, run_name="tfidf_logistic_regression"):
