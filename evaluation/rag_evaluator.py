...
# Reconfigure stdout/stderr to use UTF-8 to prevent UnicodeEncodeError on Windows terminals when printing emojis
...
from ml.experiment_tracking import log_rag_eval_run
    ...
    for i, tc in enumerate(test_cases):
        ...
        try:
            result = rag_chain.invoke(
                {"input": tc["question"]}, config={"configurable": {"session_id": f"eval-{tc['id']}"}}
            )
            ...
            print(
                f"  {status} Correctness={correctness['score']:.1f} | Relevance={relevance['score']:.1f} | Faithfulness={faithfulness['score']:.1f} | {elapsed:.1f}s"
            )
...
def print_report(evaluation: dict):
    ...
    for cat, scores in sorted(agg["scores_by_category"].items()):
        print(
            f"    {cat:<20} {scores['answer_correctness']:>8.2f} {scores['context_relevance']:>9.2f} {scores['faithfulness']:>9.2f} {scores['count']:>6}"
        )

    # Flag worst performing cases
    print("\n  Lowest Scoring Cases:")
    results_with_avg = [(r, sum(r["scores"].values()) / 3) for r in evaluation["results"] if "error" not in r]
...
def main():
    ...
    # Fetch parameters to log
    retrieval_strategy = "hybrid"
    k = 3
    prompt_template_name = "Structured"

    rag_chain = RAGChainBuilder(vector_store, retrieval_strategy=retrieval_strategy).build_chain()
    print("  Ready!")
    ...
    # Log to MLflow
    print("\nLogging RAG evaluation run to MLflow...")
    log_rag_eval_run(
        retrieval_strategy=retrieval_strategy,
        k=k,
        prompt_template_name=prompt_template_name,
        metrics=evaluation["aggregate"]["avg_scores"],
        run_name=f"rag_eval_{retrieval_strategy}_k{k}",
